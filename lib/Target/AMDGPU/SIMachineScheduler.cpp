...
SIScheduleBlock *SIScheduleBlockScheduler::pickBlock() {
  ...
  if (CurrentPathRegUsage.empty()) {
    ...
    } else if (VregCurrentUsage > 40 && Variant == SISchedulerBlockSchedulerVariant::BlockLatencyRegUsage2) {
  ...
  for (std::vector<SIScheduleBlock*>::iterator I = ReadyBlocks.begin(),
    ...
    if (VregCurrentUsage > 120 ||
        (Variant != SISchedulerBlockSchedulerVariant::BlockLatencyRegUsage &&
         Variant != SISchedulerBlockSchedulerVariant::BlockLatencyRegUsage2)) {
...
void SIScheduleDAGMI::schedule()
{
  ...
  unsigned BestScore = INT_MAX;

  std::vector<std::pair<SISchedulerBlockCreatorVariant, SISchedulerBlockSchedulerVariant>> Variants_main = {
      { LatenciesGrouped, BlockLatencyRegUsage },
      { LatenciesGrouped, BlockLatencyRegUsage2 },
      { LatenciesAlone, BlockLatencyRegUsage },
      { LatenciesAlone, BlockLatencyRegUsage2 },
      { LatenciesAlonePlusConsecutive, BlockLatencyRegUsage2 },
      { LatenciesAlonePlusConsecutive, BlockLatencyRegUsage }
    };
    for (std::pair<SISchedulerBlockCreatorVariant, SISchedulerBlockSchedulerVariant> v : Variants_main) {
      unsigned Score;
      Temp = Scheduler.scheduleVariant(v.first, v.second);
      Score = Model.getCycleCountForInstrExecInfo(
        Model.getInstrExecInfoForSchedule(convertToSUnits(Temp.SUs),
                                          SITRI->getWaveFrontsForUsage(AMDGPUSubtarget::VOLCANIC_ISLANDS,
                                                                       Temp.MaxSGPRUsage, Temp.MaxVGPRUsage)));
      Score += 10 * SITRI->spillCost(AMDGPUSubtarget::VOLCANIC_ISLANDS, Temp.MaxSGPRUsage, Temp.MaxVGPRUsage);
      DEBUG(dbgs() << "Score: " << Score << '\n');
      if (Score < BestScore) {
        Best = Temp;
        BestScore = Score;
      }
  ...
  DEBUG(dbgs() << "Best Score: " << BestScore << '\n');
