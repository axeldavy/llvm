...
void SIScheduleBlock::tryCandidateTopDown(SISchedCandidate &Cand,
  ...
  if (tryLess(TryCand.HasLowLatencyNonWaitedParent,
              Cand.HasLowLatencyNonWaitedParent, TryCand, Cand, SIScheduleCandReason::Depth))
    return;
  ...
  for (unsigned i = 0, e = DAGSize; i != e; ++i) {
    ...
    for (SUnit* SU : SUs) {
      ...
      if (&*CurrentTopFastSched == MI) {
        ...
        // It would gain a lot if there was a way to recompute the LiveIntervals
        // for the entire scheduling region.
...
void SIScheduleBlockCreator::fillStats() {
  ...
  std::vector<int> TopDownIndex2Indice;
  std::vector<int> TopDownIndice2Index;
  std::vector<int> BottomUpIndex2Indice;

  WorkList.reserve(DAGSize);

  TopDownIndex2Indice.resize(DAGSize);
  TopDownIndice2Index.resize(DAGSize);
  BottomUpIndex2Indice.resize(DAGSize);
  ...
  BottomUpIndex2Indice = std::vector<int>(TopDownIndex2Indice.rbegin(), TopDownIndex2Indice.rend());
  ...
  for (unsigned i = 0, e = DAGSize; i != e; ++i) {
    int BlockIndice = BottomUpIndex2Indice[i];
    SIScheduleBlock *Block = CurrentBlocks[BlockIndice];
...
SIScheduleBlockScheduler::SIScheduleBlockScheduler(SIScheduleDAGMI *DAG,
  ...
  // TODO: the sort is duplicated from scheduleInsideBlocks
  // We could reuse the previous result.
  std::vector<int> WorkList;
  std::vector<int> TopDownIndex2Indice;
  std::vector<int> TopDownIndice2Index;
...
bool SIScheduleBlockScheduler::tryCandidateLatency(SIBlockSchedCandidate &Cand,
  ...
  if (tryGreater(TryCand.IsHighLatency, Cand.IsHighLatency,
    ...
    return true;
  if (TryCand.IsHighLatency && tryGreater(TryCand.Height, Cand.Height, TryCand, Cand, Depth))
    return true;
...
void SIScheduleDAGMI::schedule()
{
  ...
  // if VGPR usage is extremely high, try other good performing variants which could lead to lower VGPR usage
...
//      { LatenciesAlonePlusConsecutive, BlockRegUsage }
  ...
  // if VGPR usage is still extremely high, we may spill. Try other variants which are less performing,
  // but that could lead to lower VGPR usage.
